namespace Minimercado
{
    ...
    public class AutoCompleteTextbox : TextBox
    {
        ...
        // the list for our suggestions database
        public DataTable AutoCompleteList
        {
            get;
            set;
        }
        ...
        private void UpdateCurrentAutoCompleteList()
        {
            ...
            foreach (DataRow fila in AutoCompleteList.Rows)
            {
                // be casesensitive
                if (CaseSensitive)
                {
                    // search for the substring (equal to SQL Like Command)
                    if ((fila["RASOCLIE"].ToString().IndexOf(this.Text) > -1))
                    // Add candidates to new CurrentAutoCompleteList
                    { CurrentAutoCompleteList.Add(fila["RASOCLIE"].ToString() + "|" + fila["IDCLIE"].ToString()); }
                }
                // or ignore case
                else
                {
                    // and search for the substring (equal to SQL Like Command)
                    if ((fila["RASOCLIE"].ToString().ToLower().IndexOf(this.Text.ToLower()) > -1))
                    // Add candidates to new CurrentAutoCompleteList
                    { CurrentAutoCompleteList.Add(fila["RASOCLIE"].ToString() + "|" + fila["IDCLIE"].ToString()); }
                }
            }
        ...
        private void UpdateListBoxItems()
        {
            ...
            if ((ParentForm != null))
            {
                try
                {
                    ...
                    //panel.Location = this.Location + new Size(0, this.Height) + new Size(12, 13);
                    if (aux == 0)
                        panel.Location = this.Location + new Size(0, this.Height);
                    else
                        panel.Location = new Point(aux, this.Location.Y + 25) + new Size(0, this.Height);
